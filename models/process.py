...
def inference(model, data_set, dataloader, device, node_cnt, window_size, horizon):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(dataloader):
            ...
            while step < horizon:
                forecast_result, a = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
...
def validate(model, data_set,dataloader, device, normalize_method,
    ...
    if result_file:
        ...
        step_to_print = 0
        forcasting_2d = forecast_norm[:, step_to_print, :]
        forcasting_2d_target = target_norm[:, step_to_print, :]
    ...
    if data_set.set_type == 2:
        forecast_raw = data_set.inverse_transform(forcasting_2d)
        target_raw = data_set.inverse_transform(forcasting_2d_target)
        np.savetxt(f'{result_file}/forecast_raw.csv', forecast_raw, delimiter=",")
        np.savetxt(f'{result_file}/target_raw.csv', target_raw, delimiter=",")
...
def train(args, train_set, train_loader,valid_set, valid_loader, result_file):
    ...
    model = Model(args, node_cnt, 1, args.window_size, args.multi_layer, horizon=args.horizon)
    ...
    for epoch in range(args.epoch):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            model.zero_grad()
            forecast, _ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
