...
parser.add_argument('--dataset_path', type=str, default='custom.csv',required=True)
...
# according to the feature of the dataset
parser.add_argument('--node_cnt', type=int, default=12,required=True)
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        rmse = test(args, test_set, test_loader,result_train_file, result_test_file)
predict_file = os.path.join(result_test_file, 'predict.csv')
target_file = os.path.join(result_test_file, 'target.csv')
pre = pd.read_csv(predict_file, header=None)
tar = pd.read_csv(target_file, header=None)
computeindicator(tar, pre).compute()
